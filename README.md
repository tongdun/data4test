...
- 6、国际化（i18n）体系
  - 多语种界面：系统文案、菜单标题支持中英文等多语种切换，无需重新编译
  - 多语种业务数据：测试用例、数据文件、场景、任务的名称/描述/步骤等字段按语种翻译，一次建设、多语种复用
  - i18n 同步：从库中一键导出中文骨架（zh-CN.yaml），翻译后补充其他语种文件（如 en-US.yaml）即自动生效
...
#### 社区微信群
- 如需加入我们的开源社区进行沟通，请关注微信公众号后，后台发送"加入社群"，即可获得入群邀请码
