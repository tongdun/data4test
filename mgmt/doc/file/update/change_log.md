#### 2026年9月16日
1、[Optimize] 测试用例i18n同步增加备注字段的同步、翻译
2、[Optimize] 数据、场景执行历史查看详情增加任务ID信息展示
3、[Bug] 任务报告场景关联数据按时序排序存在乱充，改为按id排序
4、[Optimize] 任务报告已执行的场景增加未执行数据明细的展示

#### 2026年9月16日
1、[Optimize] 结果报告列表「关联任务」字段增加模糊过滤，支持按任务 ID / 任务 tag 筛选报告
2、[Optimize] 单任务/多任务报告交互样式优化
3、[Bug] 全局弹出式子框确认后会跳转去其他页面修复

#### 2026年9月15日
1、[Feature] 用例统计定义列表增加一键导出测试Excel用例的功能
2、[Optimize] 用例统计报告、定时任务报告风格统一

#### 2026年9月12日
1、[Bug] 用例统计报告「按模块统计」中「未执行」数字超链下钻过滤不到数据：原用 test_result=untest 精确值过滤，漏掉空值/杂值状态的未执行用例；改为查询层补集过滤（test_result 非 pass/part/fail/deprecated/unmerged 均视为未执行），不改库内值

#### 2026年9月10日
1、[Bug] 测试用例 Excel 导入时测试时间与填写不一致：新增测试时间归一化，兼容 Excel 日期序列号与常见日期格式，统一落库为 2006-01-02 15:04:05 格式
2、[Feature] 测试用例导入模板下载附带一条示例数据行；用例类型、测试结果、产品线列生成下拉选项；测试时间列设为文本格式避免 Excel 自动转日期序列号
3、[Optimize] 测试用例导入示例行（用例编号以「示例」开头）自动跳过不落库；测试结果归一化支持中英文枚举且空值默认「未测试(untest)」
4、[Bug] 测试用例 Excel 导入 WPS「嵌入单元格图片」显示为 =DISPIMG(...) 公式而非图片：新增解析 xl/cellimages.xml 反查图片，导入时提取 WPS 嵌入图片并跳过公式文本
5、[Bug] 测试过程字段在模板最后一列时，其右侧溢出单元格内的 WPS 嵌入图片导入后丢失：改为测试过程列及其右侧所有列逐列提取 DISPIMG 图片，多图一并导入

#### 2026年9月8日
1、[Feature] 测试用例新增结果历史记录：结果发生变化时把旧结果追加进历史（执行次数/执行结果/执行人员/执行时间），首次执行不写历史，最新结果仍保留在 test_result 字段
2、[Feature] 测试用例列表新增批量修改：可对选中用例批量修改 用例模块/测试结果/引入版本/功能开发者/用例设计者/用例执行者/测试时间（测试结果变化沿用结果历史记录逻辑；模块变更时自动迁移测试过程图片到新模块目录）
3、[Bug] 测试用例导入 Excel 时测试结果未归一化导致列表显示为空；新增结果值映射：PASS/Pass/通过→pass、FAIL/Fail/失败→fail、N/A/NA/已废弃/不适用→deprecated、暂未合入→unmerged（未知值原样保留）
4、[Optimize] 测试用例「关联产品」数据源由产品表改为系统参数 caseProduct（支持多选，多个产品逗号分隔存储）；Excel 导入页产品下拉同源，文案统一为「关联产品」
...
2、[Bug] 数据列表测试，当数据没有API_ID时，会显示找不到数据, 非接口类的数据本就无API_ID
3、[Bug] 任务导入数据包，数据列表中标准数据接口ID、关联应用没有信息
4、[Bug]场景列表页面有任务执行ID列，应该无
5、[Optimize] 场景历史、数据历史列表，task_id 字段位置移到 name 之后

#### 2026年8月6日
1、[Feature] 增加接口获取登录TOKEN信息，CLI可通过接口获取鉴权TOKEN
2、[Bug] CLI调用接口无法识别非标准文件的执行，进行修复

#### 2026年8月5日
1、[Feature] 任务数据导入增加单独的页面

#### 2026年8月4日
1、[Feature] 新增接口支持CLI友好调用，临时支持数据和场景的执行
2、[Feature] 增加断言动作output2auth,自动更新环境鉴权信息

#### 2026年8月3日
1、[Optimize] 导出任务数据包和知识库数据时，导出文件变成yaml，且导入同步变更，CLI友好

#### 2026年7月31日
1、[Bug] 结果断言属性定位带||时，值为整数时，无法正常定位

#### 2026年7月29日
1、[Feature] 任务列表增加数据包导入功能

#### 2026年7月28日
1、[Feature] 未执行任务查看报告时，可查看到未执行报告的信息，方便查看任务覆盖范围
2、[Feature] 参数页面增加获 取知识库数据包的功能
3、[Bug] 任务导出时，国际化文案修复

#### 2026年7月23日
1、[Optimize] 场景历史列表默认只展示前7列，按需选择其他列进行展示，支持缓存  

#### 2026年7月22日
1、[Feature] 支持国际化，支持英文和中文  
2、[Feature] 任务增加报告查看，分单任务报告和多任务报告  
3、[Optimize] 应用、产品、全局维度的报告改成离线模式，后台生成再查看  

#### 2026年6月8日
1、[Bug] 数据列表选择环境进行测试，无法选择到新增的环境，进行修复

#### 2026年6月5日
1、[Bug] 场景历史列表，再来一次，找不到历史数据文件，进行修复
2、[Bug] 场景历史列表，继续功能，修复一波

#### 2026年6月3日
1、[Bug] 当场景找不到关联的数据时，查看场景详情结果，会出现范围越界错误，进行修复

#### 2026年6月2日
1、[Bug] 断言比较值没有四则运算符的不再过四则运算，会导致000类的数字字符串无法正常比较
2、[Bug] 断言比较按属性取原数据时，当属性值为数字时，定义是字符串，值是float,导致比较失败，统一改成string进行比较
3、[Bug] 断言比较值存在小数型时，比较结果不正确，统一修复

#### 2026年4月24日
1、[Optimize] 产品配置下的专用参数改为不支持多值，不再做多值的切割，如需多值，统一在系统参数中定义

#### 2026年4月23日
1、[Feature] 产品配置新增专用应用前缀定义, 支持一个应用在不同环境有不同前缀的情况
2、[Optimize] 任务一键导出产品时，同步增加产品应用前缀信息的导出

#### 2026年4月21日
1、[Optimize] 场景列表，任务列表关联数据或场景时，选择模式先屏蔽

#### 2026年4月20日
1、[Feature] 测试结果记录增加操作人记录，方便数据过滤和查找
2、[Optimize] 场景、数据列表，结果列表，过滤搜索框默认展开

#### 2026年4月19日
1、[Feature] 支持ResponseHeader头断言，数据源定义(source)为ResponseHeader:keyName，或ResponseHeader:regexMatchStr, 用正则匹配取值
2、[Optimize] 断言值为output_re时，不再进行JSON格式化，直接进行正则匹配提取，解决当返回信息非JSON字符串时，断言异常的问题
3、[Bug] 任务一键导出时，数据文件包遇到数据描述为空时，导出文件会异常，增加判断进行兼容处理
4、[Optimize] 系统自动生成任务，创建人需要变更为创建人，非系统用户，以示区分
5、[Bug] 测试数据生成测试任务时，查看任务详情异常，问题修复
6、[Feature] 测试数据支持一建生成测试场景

#### 2026年4月17日
1、[Optimize] 生成知识库场景时，会出现一个空数据占位符，进行判断去除
2、[Feature] 任务列表一键导出，支持场景文件导出，打成压缩包

#### 2026年4月16日
1、[Bug] 任务列表一键导出只选单个任务时，导出文件没有下载链接问题修复
2、[Feature] 任务列表一键导出，支持数据文件包导出，打成压缩包

#### 2026年4月14日
1、[Feature] 自定义脚本支持出参提供，以格式output: {"key1": ["value1", "value1"], "key2": ["value1", "value1"]} 的形式提供，供后续数据使用

#### 2026年4月13日
1、[Optimize] 关联产品信息，逻辑改成按更新时间倒排

#### 2026年4月10日
1、[Feature] 生成测试数据，支持直接导入断言信息
2、[Bug] 智能数据导入时关联生成平台台没有实时加载
3、[Bug] 断言导入时，断言类型没有对
4、[Bug] 智能场景取用时，智能数据没有同步取用

#### 2026年1月15日
1、[Feature] 用例导出为Xmind格式，支持根据条件过滤导出，范围：智能用例、测试用例

#### 2026年1月5日
1、[Bug] 任务一键导出，文件下载链接多了一个空格，导致下载找不到路径

#### 2025年12月29日
1、[Optimize] 全局任务、场景列表变更产品时，只下拉关联最近一年有更新的产品信息，方便快速选择
2、[Optimize] 大盘统计数据，只统计最近一年有更新的产品信息，加快统计速度

#### 2025年12月25日
1、[Feature] Xmind转Excel支持测试结果和标签的识别

#### 2025年12月23日
1、[Optimize] 导出文件时，直接提供文件链接，复制进行下载, 涉及: 测试用例、智能用例、任务等导出功能

#### 2025年10月30日
1、[Optimize] Enum枚举参数，支持小数类型

#### 2025年9月9日
1、[Bug] 多轮会话如遇错，后续会话变成新的会话，问题修复

#### 2025年9月5日
1、[Feature] 智能模板增加复制功能
2、[Bug] 导出Xmind用例，部分用例导出格式异常修正

#### 2025年8月25日
1、[Bug] 历史数据带<pre><code>标准的数据直接从数据库取执行异常
2、[Bug] 场景从未在新环境执行过时，任务执行时会显示找不到文件问题修复
3、[Bug] 数据历史列表再来一次执行异常修复

#### 2025年8月18日
1、[Bug] 测试用例列表前置条件增加注入攻击内容的转义
2、[Bug] 生成数据/用例等子弹出框关联产品/模板等无最新的数据，全局修复
3、[Bug] 子弹出框保存后url会跳到其他列表页，全局修复
4、[Bug] 导出Xmind，测试步骤只有单条时，未正常导出
