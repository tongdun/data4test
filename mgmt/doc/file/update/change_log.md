##### 2024年2月~5月
1. [Feature] 场景类型增加串行继续，并发比较，普通并发等多种类型
2. [Bug] 场景列表修改时，备注不生效, 备注remark被不小心改成了remark1导致
3. [Bug] 生成excel文件，同样的信息内容会多一倍，逻辑写的有问题
4. [Bug] 获取身份证号的省份，城市，国籍的信息在Mock接口未返回具体数据,路由参数绑定名称错误导致
5. [Bug] 生成数据的方案因为占位符的替换方案变更，导致数据不再随机生成,重新设计方案合入
6. [Feature]支持生成txt格式文件数据
7. [Optimize]生成文件数据和记录文件数据代码逻辑优化
8. [Feature] Single下的参数，JSON格式时，多层级变量定义在专用参数里支持正常替换
9. [Optimize]通过数据包导入数据用例的功能先进行屏蔽
10. [Feature]各类Mock数据增加Query参数请求的方式
11. [Feature] 任务/场景执行时，支持把多环境关联和执行
12. [Bug] 串行继续/并发类型执行的场景，执行失败后，结果每失败一次会记录一次，记录重复
13. [Bug] 场景测试，查看结果记录，串行继续和并行模式最近数据文件传参顺序错误，导致记录为playbook
14. [Bug] 场景结果记录，串行继续的场景记录为了串行中断，为定义文案提供错误
15. [Bug] 任务测试，查看场景结果记录，串行继续和并行模式最近数据文件传参顺序错误，导致记录为task（排查中）
16. [Bug] 任务执行，部分数据替换变量时，定位超出索引，正则匹配加强
17. [Feature] 断言变量统一替换（doing）
18. [Optimize]任务列表，关联产品，下拉列表非最新的数据在最前
19. [Optimize]数据历史，场景历史，关联产品，最新产品不在最前
20. [Optimize]当断言比较类型为raw时，失败时返回信息过多，原返回信息不在错误中展示，使用Response代号
21. [Optimize] 出现错误时，日志信息进行优化，方便问题定位
22. [Bug]控制台数据域请求，请求数据中，Request, 响应数据中没有信息返回
23. [Optimize]控制台数据域请求，请求数据中，Request, 响应数据进行格式化展示
24. [Optimize]查看数据历史域请求数据，返回数据进行格式化层级展示
25. [Bug]查看数据历史域数据，header头信息为空，修复
26. [Optimize]断言计算符=，>等取值后需要能直接进行计算比较
27. [Optimize]断言失败信息需要展示更详细，方便问题排查和定位
28. [Optimize]接口自动化数据同步处理
29. [Bug]数据历史未执行的数据用例跳转链接不对，超链修复
30. [Bug]场景历史无名称时，自动填充，最近文件未执行的正常展示
31. [Feature] 导出文件支持自动化校验，已实现：CSV/EXCEL/YAML/JSON
32. [Bug]导入接口，新增接口未正常录入，是否自动化默认值变更导致写入失败
33. [Optimize]导出文件为后端返回文件名称时，方案优化，减少数据用例编写成本
34. [Bug]从场景列表测试后全部通过，场景历史结果最近数据文件应该为空，非playbook, 传参错误导致
35. [Bug]从数据列表发起的测试，数据历史列表，超链路径少了目录名称的信息，导致跳转不对
36. [Bug]从数据列表发起的测试，数据列表的文件未记录实时的结果
37. [Bug]数据列表，增加YAML数据文件，内容带XML格式时，无法原样保存
38. [Optimize] 数据列表，保存数据文件历史版本逻辑优化，历史版本单独存目录
